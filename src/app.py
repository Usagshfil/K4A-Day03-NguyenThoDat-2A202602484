        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            if not obs_data:
                ...
                trace_logs.append({
                    ...
                    "query": user_query,
                    "action_type": "FINAL_ANSWER",
                    "thought": "Lỗi dữ liệu từ MCP Server.",
                    "output": final_answer,
                    "latency_ms": 10.0
                })
                break
                ...
                history_events.append(
                    ...
                    f"- Bước {step} Observation: {obs_str}"
                )

                # Nạp Observation cho lượt suy luận kế tiếp
                current_prompt = (
                    f"Yêu cầu ban đầu của người dùng: {user_query}\n\n"
                    f"Lịch sử thực thi các bước ReAct trước đó:\n" + "\n".join(history_events) + "\n\n"
                    f"Chỉ dẫn:\n"
                    f"- Nếu dữ liệu thu thập được đã đủ để trả lời hoàn chỉnh yêu cầu ban đầu, hãy đưa ra câu trả lời cuối cùng (Final Answer) bằng văn bản rõ ràng, lịch sự, chính xác và không bịa đặt dữ liệu.\n"
                    f"- Nếu vẫn cần thêm thông tin hoặc cần thực hiện thêm bước tiếp theo (ví dụ: sau khi tra cứu thông tin cố vấn thì cần đặt lịch hẹn với cố vấn đó), hãy tiếp tục gọi công cụ tương ứng."
                )
